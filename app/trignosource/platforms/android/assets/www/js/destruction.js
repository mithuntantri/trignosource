...
var app = {
    ...
    initialize: function() {
        ...
        this.transactionObject[4].values.push({
            "asset": null,
            "values": null
        })

        for(var i=0;i<this.transactionObject[4].values.length;i++){
            final_html += `<div class="purchase-item-block">
                                <div class="title">Theft / Destruction ${i+1}</div>`
                                if(i != this.transactionObject[4].values.length-1){
                                ...
                                final_html += `<div class="detailed-transaction-block" style="display:none;padding:15px;width:90%;" id="purchase_form_${i}">
                                                         <div class="title">
                                                            ${this.currentAsset} thieved / destroyed:
                                                         </div>
                                                         <div class="input-container" style="margin:5px 0 15px 0;position: relative;">
                                                             <input class="select-box grey-box" placeholder="Select the asset thieved / destroyed" 
                                                                id="asset_${i}" disabled type="text" 
                                                                style="font-size:19px;"  `
                                                                     if(that.transactionObject[4].values[i].asset){
                                                                        final_html += `value="${that.transactionObject[4].values[i].asset}"`
                                                                     }
                                                                final_html += `/>
                                                        ...
                                                        <div class="calculate-btn green-btn" id="create_btn_${i}" 
                                                            ...
                                                            if(i == that.transactionObject[4].values.length -1){
                                                                final_html += `<span class="c-text">CREATE THEFT / DESTRUCTION</span>`
                                                            }else{
        ...
        for(var i=0;i<this.transactionObject[4].values.length;i++){
            (function(i){
                ...
                purchase_create.addEventListener('click', function(){
                    ...
                    if(j == that.transactionObject[4].values.length-1){
                ...
                purchase_delete.addEventListener('click', function(){
                    ...
                    if(j == that.transactionObject[4].values.length-1){
                        ...
                        that.transactionObject[4].values.splice(j,1)
                        that.transactionObject[4].values.splice(that.transactionObject[4].values.length-1, 1)
                        that.transactionObject[4].count = that.transactionObject[4].values.length
                ...
                for(var k=0;k<assets.length;k++){
                    (function(k){
                        ...
                        month.addEventListener('click', function(event){
                            ...
                            if(document.getElementById('asset_'+j).value == "General / Other"){
                                that.transactionObject[4].values[j].values = {
                                    "sale_proceeds": null,
    ...
    addWholeSale: function(i, l, option){
        ...
        final_html += `  <div class="title">
                         ...
                         <div style="display:flex;flex-direction:row;justify-content:space-between;">
                             <div class="input-container" style="margin:5px 0 15px 0;position: relative;width:70%;">
                                 <input class="select-box grey-box" placeholder="Date & Month" id="sale_month_${i}" disabled type="text" 
                                     style="font-size:19px;" `
                                     if(that.transactionObject[4].values[i].values.date_of_sale){
                                        final_html += `value="${that.transactionObject[4].values[i].values.date_of_sale}"`
                                     }
                                final_html += `/>
                            ...
                            <div class="input-container" style="margin:5px 0 15px 5px;position: relative;flex-grow:2;">
                                <input class="select-box grey-box" placeholder="Year" id="sale_year_${i}" disabled type="number"
                                    style="font-size:19px;width:100%;" value="${that.transactionObject[4].values[i].values.year_of_sale}"/>
                        ...
                        <div class="input-container" style="margin:5px 0 15px 0;">
                             ...
                             type="number" value="${that.transactionObject[4].values[i].values.sale_proceeds}" />
                        </div>`

        if(!option){
            final_html += `<div class="title">
                           Cost of ${that.currentAsset} theived / destroyed:
                        </div>
                        <div class="input-container" style="margin:5px 0 15px 0;">
                             <div class="input-unit green-unit">&#8377;</div>
                             <input class="input-box grey-box" id="cost_of_asset_${i}" 
                             type="number" value="${that.transactionObject[4].values[i].values.purchase_cost}" />
                        </div>`
        }
    ...
    addAssetForm: function(i, l){
        ...
        var extents = [
            "Theft / Destruction of whole " + this.currentAsset,
            "Theft / Destruction of part"
        ]

        final_html += ` <div class="title">
                            Extent of theft / destruction:
                         </div>
                         <div class="input-container" style="margin:5px 0 15px 0;position: relative;">
                             <input class="select-box grey-box" placeholder="Select the extent of theft / destruction" id="extent_of_sale_${i}" disabled type="text" 
                                 style="font-size:19px;" `
                                 if(that.transactionObject[4].values[i].values.extent_of_sale){
                                    final_html += `value="${that.transactionObject[4].values[i].values.extent_of_sale}"`
                                 }
                            final_html += `/>
        ...
        for(var k=0;k<extents.length;k++){
            (function(k){
                ...
                extent.addEventListener('click', function(event){
                    ...
                    if(l == 0){
                        that.transactionObject[4].values[i].values = {
        ...
        if(that.transactionObject[4].values[i].values.extent_of_sale){
            if(that.transactionObject[4].values[i].values.extent_of_sale.includes("whole")){
                that.addWholeSale(i, 0, true)
            }else if(that.transactionObject[4].values[i].values.extent_of_sale.includes("part")){
    ...
    addOPForm: function(i){
        ...
        final_html += ` <div class="title">
                        ...
                        <div class="title">
                           Cost of ${that.currentAsset} thieved / destroyed:
                        </div>
                        <div class="input-container" style="margin:5px 0 15px 0;">
                             <div class="input-unit green-unit">&#8377;</div>
                             <input class="input-box grey-box" id="cost_of_asset_${i}" 
                        ...
                        <div class="title">
                            Date of Purchase of ${that.currentAsset} thieved / destroyed:
                         </div>
                         <div style="display:flex;flex-direction:row;justify-content:space-between;">
                             <div class="input-container" style="margin:5px 0 15px 0;position: relative;width:70%;">
                                 <input class="select-box grey-box" placeholder="Date & Month" id="purchase_month_${i}" disabled type="text" 
                                     style="font-size:19px;" `
                                     if(that.transactionObject[4].values[i].values.date_of_purchase){
                                        final_html += `value="${that.transactionObject[4].values[i].values.date_of_purchase}"`
                                     }
                                final_html += `/>
                            ...
                            <div class="input-container" style="margin:5px 0 15px 5px;position: relative;flex-grow:2;">
                                <input class="select-box grey-box" placeholder="Year" id="purchase_year_${i}" disabled type="number"
                                    style="font-size:19px;width:100%;" value="${that.transactionObject[4].values[i].values.year_of_purchase}"/>
    ...
    createSale: function(i){
        ...
        if(document.getElementById('asset_'+i).value == null || document.getElementById('asset_'+i).value == ''){
            alert("Please select the "+this.currentAsset+" destroyed/thieved to fill in further details and later click on the CREATE THEFT/DESTRUCTION button.")
            stop = true
        }else if(document.getElementById('asset_'+i).value == "General / Other"){
            var sale_proceeds = parseFloat(document.getElementById('sale_proceeds_'+i).value)
            console.log("sale_proceeds", sale_proceeds)
            if(sale_proceeds == null || sale_proceeds == '' || isNaN(sale_proceeds)){
                sale_proceeds = 0
            }
            that.transactionObject[4].values[i] = {
                "asset": document.getElementById('asset_'+i).value,
                "values": {
                    "sale_proceeds": parseFloat(sale_proceeds.toFixed(2)),
                    "date_of_purchase": document.getElementById('purchase_month_'+i).value,
                    "year_of_purchase": parseInt(document.getElementById('purchase_year_'+i).value),
            ...
            if(document.getElementById('extent_of_sale_'+i).value == null || document.getElementById('extent_of_sale_'+i).value == ''){
                alert("Please select the extent of theft/destruction to fill in further details and later click on the CREATE THEFT/DESTRUCTION button")
                stop = true
    ...
    checkForm: function(j){
        let obj = this.transactionObject[4].values[j]
        if(obj.values.date_of_sale == null || obj.values.date_of_sale == ''){
            alert("Please select the date and month of theft/destruction")
            return false
        }else if(obj.values.year_of_sale == null || obj.values.year_of_sale == ''){
            alert("Please select the year of theft/destruction")
            return false
        }else if(obj.asset == "General / Other"){
            if(obj.values.purchase_cost == 0){
                alert("Please enter the cost of "+this.currentAsset+" thieved/destroyed")
                return false
            }else if(obj.values.date_of_purchase == null || obj.values.date_of_purchase == ''){
                alert("Please select the date and month of purchase, of "+this.currentAsset+" thieved/destroyed.")
                return false
            }else if(obj.values.year_of_purchase == null || obj.values.year_of_purchase == ''){
                alert("Please select the year of purchase, of "+this.currentAsset+" thieved/destroyed.")
                return false
            }else{
                return true
            }
        }else if(obj.asset != "General / Other"){
            if(obj.values.asset_cost <= obj.values.purchase_cost){
            ...
            if(obj.values.sale_proceeds == null || obj.values.sale_proceeds == ''){
                this.transactionObject[4].values[j].sale_proceeds = 0
            }
            return true
        }
    },
