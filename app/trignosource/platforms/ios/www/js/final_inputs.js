...
var app = {
    ...
    updatePurchaseSchedule: function(asset_number, d, closing_date, opening_date){
        var i = 1, 
        min = this.schedules.purchase[asset_number][i].year_begining
        max = this.schedules.purchase[asset_number][this.schedules.purchase[asset_number].length-1].year_begining
        for(j=min;j<=parseInt(max);j++){
            var schedule = _.clone(this.schedules.purchase[asset_number])
            var purchase = {
                opening_date : opening_date,
                year_begining: schedule[i].year_begining,
                opening_balance: schedule[i-1].closing_balance,
                depreciation: schedule[i-1].closing_balance * d,
                closing_balance: schedule[i-1].closing_balance - (schedule[i-1].closing_balance * d),
                closing_date: closing_date,
                year_end: schedule[i].year_begining,
            }
            console.log("purchase", i, purchase)
            this.schedules.purchase[asset_number][i] = purchase
            i++
        }
        console.log("updated Purchase schedule", schedule)
    },
    ...
    updatePurchaseScheduleSalePart: function(asset, sale, pos, closing_date, opening_date, d){
        console.log("Inside updatePurchaseScheduleSalePart")
        let asset_number = parseInt(asset.split(" ")[1])-1
        console.log("asset_number", asset_number)
        console.log(sale)
        for(i=0;i<this.schedules.purchase[asset_number].length;i++){
            let obj = this.schedules.purchase[asset_number][i]
            console.log("obj", obj)
            if(obj.year_begining == ((closing_date == '31st March')?sale.apsc5[pos]-1:sale.apsy[pos])){
                console.log("Found year", sale.ayopsob[pos])
                let new_op = this.schedules.purchase[asset_number][i].opening_balance - sale.ayopsob[pos]
                this.schedules.purchase[asset_number][i].opening_balance = new_op
                this.schedules.purchase[asset_number][i].depreciation = new_op * d
                this.schedules.purchase[asset_number][i].closing_balance = new_op - (new_op * d)
            }
        }
        this.updatePurchaseSchedule(asset_number, d, closing_date, opening_date)
    },
    ...
    createAssetAccount: function(closing_date, opening_date, d){
        ...
        for(var i=0;i<this.transactionVariables.purchase.apm.length;i++){
            ...
            obj.all.push(acc)
            depreciation_account.all.push(acc)
            ...
            }else if(this.transactionObject[6].values.length > i && this.transactionObject[6].values[i].extent_of_sale.includes("whole") && this.transactionObject[6].values[i].values.extent_of_sale == "Significant"){
            ...
            }else if(this.transactionObject[6].values.length > i && this.transactionObject[6].values[i].extent_of_sale.includes("part") && this.transactionObject[6].values[i].values.extent_of_sale == "Significant"){
            ...
            }else if(this.transactionObject[6].values.length > i && this.transactionObject[6].values[i].extent_of_sale.includes("part") && this.transactionObject[6].values[i].values.extent_of_sale == "Negligible"){
                pnl_account.push({
